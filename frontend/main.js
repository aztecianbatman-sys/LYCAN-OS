...
function findFirefox() { return candidateFirefoxPaths().find(p => fs.existsSync(p)) || null; }
...
function launchPackage(id) {
  const { file, manifest, permissions, dir } = packageEntryPath(id);
  const partition = `persist:lycan-${id}`;
