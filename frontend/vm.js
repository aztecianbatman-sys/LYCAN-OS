...
class LycanVM {
  ...
  cmd(command) {
    ...
    switch (verb.toLowerCase()) {
      ...
      case 'help': return 'help | version | memory | vm pages | ps | ls <path> | tree <path> | write <path> <text> | network status|on|off | web status | diagnostics | apps | app register|unregister | open|close|suspend|resume|crash <id> | snapshots | snapshot create|restore|info|delete';
      ...
      case 'web': return `WEB BRIDGE ${rest[0] === 'start' ? 'READY' : 'READY'}\nENGINE GECKO\nNETWORK ${this.state.network.online ? 'ONLINE' : 'OFFLINE'}`;
  ...
  diagnostics() {
    ...
    let files=0; const walk=d=>{for(const e of fs.readdirSync(d,{withFileTypes:true})){const p=path.join(d,e.name);try{if(e.isDirectory())walk(p);else files++;}catch{}}}; walk(this.root);
  ...
  appCommand(args) {
    const sub=(args[0]||'').toLowerCase(), id=args[1];
  ...
  processCommand(action,args){
    const id=args[0];if(!id)throw new Error(`${action} requires an id`);let p=[...this.processes.values()].find(x=>x.id===id);
    if(!p){if(action!=='open')throw new Error('PROCESS NOT FOUND');const a=this.alloc(id,4),pid=this.nextPid++;p={pid,id,name:id,state:'RUNNING',pages:a.pages,startPage:a.start};this.processes.set(pid,p);return `OPEN ${id} PID=${pid}`;}
  ...
  snapshotCommand(args){
    ...
    if(sub==='create'){const name=(args.slice(1).join('-')||`checkpoint-${Date.now()}`).replace(/[^a-zA-Z0-9._-]/g,'-');const target=path.join(this.snapshotDir,name+'.json');const payload={format:2,createdAt:new Date().toISOString(),state:this.state,apps:this.apps,pages:Array.from(this.pages),files:this.captureFiles()};fs.writeFileSync(target,JSON.stringify(payload));return `SNAPSHOT CREATED ${name}`;}
    ...
    if(sub==='info'){const name=args[1];if(!name)throw new Error('INFO requires snapshot name');const file=path.join(this.snapshotDir,name.endsWith('.json')?name:`${name}.json`);const s=fs.statSync(file),data=JSON.parse(fs.readFileSync(file,'utf8'));return `NAME  ${name.replace(/\.json$/,'')}\nCREATED  ${data.createdAt}\nBYTES  ${s.size}\nFILES  ${Object.keys(data.files||{}).length}`;}
    if(sub==='delete'){const name=args[1];if(!name)throw new Error('DELETE requires snapshot name');const file=path.join(this.snapshotDir,name.endsWith('.json')?name:`${name}.json`);fs.rmSync(file);return `SNAPSHOT DELETED ${name}`;}
