(()=>{
  ...
  const open=()=>{shell.classList.add('visible');scene.classList.add('launcher-open');render('');setTimeout(()=>input.focus(),30)};
  shell.addEventListener('click',e=>{if(e.target.classList.contains('launcher-backdrop')||e.target.classList.contains('launcher-close'))close();const item=e.target.closest('.launcher-item');if(item)launch(item.dataset.app)});
  input.addEventListener('input',()=>render(input.value));
  input.addEventListener('keydown',e=>{if(e.key==='ArrowDown'){e.preventDefault();move(1)}if(e.key==='ArrowUp'){e.preventDefault();move(-1)}if(e.key==='Enter'){e.preventDefault();const item=visible()[selected];if(item)launch(item.dataset.app)}if(e.key==='Escape'){e.preventDefault();close()}});
