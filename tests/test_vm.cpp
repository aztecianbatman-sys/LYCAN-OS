...
int main(){
 ...
 const auto before=fs.usedBytes();const auto tampered=root/"tampered.lypkg";assert(lycan::PackageArchive::create(tampered,manifest,{{"app.bin",{'B','A','D'}}},ce));const auto recorded=lycan::PackageManager::packageSha256(tampered);std::fstream tamper(tampered,std::ios::in|std::ios::out|std::ios::binary);tamper.seekp(0,std::ios::end);const std::streamoff size=static_cast<std::streamoff>(tamper.tellp());assert(size>0);tamper.seekp(size-1,std::ios::beg);char b=0;tamper.write(&b,1);tamper.close();std::string tc="{\"apps\":[{\"id\":\"sha-test\",\"version\":\"1.0.0\",\"sha256\":\""+recorded+"\"}]}";assert(!manager.installArchiveFromCatalog(tampered,tc,&error));assert(error.find("SHA-256 verification failed")!=std::string::npos);assert(fs.usedBytes()==before);assert(manager.installed().size()==1&&manager.installed()[0].version=="2.0.0");
 ...
 const auto hostRoot=root/"host";lycan::AppHost host(hostRoot);host.boot();assert(host.packages().installed().size()==6);assert(host.packages().launcherApps().size()==6);
 assert(host.apps().surfaceFor("lycan-terminal")==lycan::AppSurface::Terminal);assert(host.apps().surfaceFor("lycan-files")==lycan::AppSurface::Files);assert(host.apps().surfaceFor("lycan-web")==lycan::AppSurface::Web);assert(host.apps().surfaceFor("lycan-snapshots")==lycan::AppSurface::Snapshots);assert(host.apps().surfaceFor("lycan-diagnostics")==lycan::AppSurface::Diagnostics);assert(host.apps().surfaceFor("crawford")==lycan::AppSurface::Crawford);
 auto openResult=host.execute("open lycan-terminal");assert(openResult.find("opened Terminal")!=std::string::npos);assert(openResult.find("surface terminal")!=std::string::npos);assert(host.apps().isOpen("lycan-terminal"));assert(host.apps().find("lycan-terminal")->windowOpen);assert(host.processes().findApp("lycan-terminal"));
 auto sessions=host.apps().sessions();assert(sessions.size()==1&&sessions[0].appId=="lycan-terminal");auto ps=host.execute("ps");assert(ps.find("lycan-terminal")!=std::string::npos&&ps.find("RUNNING")!=std::string::npos);auto sessionText=host.execute("sessions");assert(sessionText.find("terminal")!=std::string::npos&&sessionText.find("OPEN")!=std::string::npos);
 assert(host.apps().suspend("lycan-terminal"));assert(host.processes().findApp("lycan-terminal")->state=="suspended");assert(host.apps().resume("lycan-terminal"));assert(host.processes().findApp("lycan-terminal")->state=="running");assert(host.execute("close lycan-terminal")=="closed");assert(!host.apps().isOpen("lycan-terminal"));assert(!host.processes().findApp("lycan-terminal"));assert(host.apps().open("lycan-files")>=100);assert(host.apps().surfaceFor("lycan-files")==lycan::AppSurface::Files);assert(host.packages().uninstall("lycan-files"));assert(!host.packages().launcherContains("lycan-files"));
 assert(host.packages().uninstall("lycan-terminal"));assert(!host.packages().launcherContains("lycan-terminal"));assert(host.packages().installed().size()==4);
 std::cout<<"LYCAN application tests passed (SHA-256 + transactional install + DB + launcher + application manager + real app sessions + lifecycle)\n";std::filesystem::remove_all(root,ec);return 0;
}
