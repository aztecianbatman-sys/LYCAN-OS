...
int main(){
 ...
 assert(p.fs().read("/config/version").value()=="0.6.0");
 ...
 lycan::LycanStore store; assert(store.loadCatalog(catalog)); assert(store.find("demo").has_value()); assert(!store.verifyPackage("missing.lypkg",""));
