...
std::string VirtualMachine::ls(const std::string&p) const{
    ...
    for(const auto&i:std::filesystem::directory_iterator(d)){
        if(i.is_symlink()) o+="LINK "; else o+=(i.is_directory()?"DIR  ":"FILE ");
...
std::string VirtualMachine::tree(const std::string&p) const{
    ...
    std::string o=base.filename().string()+"/\n";
...
std::string VirtualMachine::snapshots() const{
    ...
    for(auto&i:std::filesystem::directory_iterator(root_/"snapshots")) if(i.path().extension()==".snap") names.push_back(i.path().stem().string());
...
std::string VirtualMachine::diagnostics() const{
    std::size_t fileCount=0,dirCount=0,bytes=0; std::error_code ec;
    ...
    std::size_t snaps=0; for(auto&i:std::filesystem::directory_iterator(root_/"snapshots",ec)) if(i.path().extension()==".snap") ++snaps;
    std::ostringstream o;
    o<<asciiLogo()<<"LYCAN DIAGNOSTIC CORE\n=====================\n"
...
std::string VirtualMachine::snapshotCreate(const std::string& name){
    ...
    std::ofstream f(snapshotPath(name),std::ios::trunc); if(!f)return "SNAPSHOT WRITE FAILED";
    ...
    f<<"\npackages="; bool first=true; for(const auto&[id,v]:installed_){if(!first)f<<";";first=false;f<<id<<","<<v;} f<<"\n";
...
std::string VirtualMachine::snapshotRestore(const std::string& name){
    if(!validSnapshotName(name))return "INVALID SNAPSHOT NAME";
    std::ifstream f(snapshotPath(name));if(!f)return "SNAPSHOT NOT FOUND";
    std::string line,header;std::getline(f,header);if(header!="LYCAN-SNAPSHOT 1")return "INVALID SNAPSHOT FORMAT";
    std::vector<Process> restored;std::map<std::string,std::string> packages=installed_;uint64_t cycles=cycles_;bool network=network_;uint32_t nextPid=nextPid_;
    while(std::getline(f,line)){
        auto eq=line.find('=');if(eq==std::string::npos)continue;auto key=line.substr(0,eq),value=line.substr(eq+1);
        try{
    ...
    cycles_=cycles;network_=network;nextPid_=nextPid;processes_=std::move(restored);installed_=std::move(packages);return "SNAPSHOT RESTORED "+name;
}
...
std::string VirtualMachine::execute(const std::string&c){
    ...
    if(c=="logo")return asciiLogo(); if(c=="pwd")return "/home"; if(c=="diagnostics")return diagnostics(); if(c=="ls")return ls("/home"); if(c.rfind("ls ",0)==0)return ls(c.substr(3));
    if(c=="tree")return tree("/home");if(c.rfind("tree ",0)==0)return tree(c.substr(5));if(c=="apps")return apps();if(c=="ps")return ps();if(c=="snapshots")return snapshots();
    if(c=="network")return network_?"NETWORK ONLINE":"NETWORK OFFLINE";if(c=="network on"){network_=true;return "NETWORK ONLINE";}if(c=="network off"){network_=false;return "NETWORK OFFLINE";}
    ...
    if(c.rfind("mkdir ",0)==0){auto p=guestPath(c.substr(6));if(!validGuestPath(p)||isGuestRoot(p))return "ACCESS DENIED";std::error_code e;std::filesystem::create_directories(p,e);return e?"MKDIR FAILED":"DIRECTORY CREATED";}
    if(c.rfind("touch ",0)==0){auto p=guestPath(c.substr(6));if(!validGuestPath(p)||isGuestRoot(p))return "ACCESS DENIED";if(std::filesystem::exists(p)&&std::filesystem::is_symlink(p))return "ACCESS DENIED: SYMLINK";std::filesystem::create_directories(p.parent_path());std::ofstream f(p,std::ios::app);return f?"FILE CREATED":"TOUCH FAILED";}
    if(c.rfind("write ",0)==0){auto rest=c.substr(6),sep=rest.find(' ');if(sep==std::string::npos)return "USAGE: write <path> <text>";auto p=guestPath(rest.substr(0,sep));if(!validGuestPath(p)||isGuestRoot(p))return "ACCESS DENIED";if(std::filesystem::exists(p)&&std::filesystem::is_symlink(p))return "ACCESS DENIED: SYMLINK";std::filesystem::create_directories(p.parent_path());std::ofstream f(p);if(!f)return "WRITE FAILED";f<<rest.substr(sep+1);return "WROTE "+std::to_string(rest.size()-sep-1)+" BYTES";}
    if(c.rfind("rm ",0)==0){auto p=guestPath(c.substr(3));if(!validGuestPath(p))return "ACCESS DENIED: OUTSIDE LYFS";if(isGuestRoot(p))return "REFUSED: LYFS ROOT IS IMMUTABLE";if(std::filesystem::exists(p)&&std::filesystem::is_symlink(p))return "REFUSED: SYMLINK";std::error_code e;auto n=std::filesystem::remove_all(p,e);return e?"REMOVE FAILED":"REMOVED "+std::to_string(n)+" ITEM(S)";}
    if(c.rfind("snapshot-info ",0)==0)return snapshotInfo(c.substr(15));if(c.rfind("restore ",0)==0)return snapshotRestore(c.substr(8));if(c.rfind("delete-snapshot ",0)==0)return snapshotDelete(c.substr(16));if(c.rfind("snapshot ",0)==0)return snapshotCreate(c.substr(9));
