...
std::filesystem::path VirtualMachine::storagePath(const std::string& id,const std::string& bucket,const std::string& path) const{
    ...
    for(const auto& part:relative) if(part.empty() || part==".") continue; else if(isPathSeparator(part.string().empty()?0:part.string()[0])) return {};
    const auto appRoot=appDataRoot(id);
    const auto bucketRoot=appRoot/bucket;
    ...
    if(hasParentTraversal(rel)) return {};
    return target;
}
uint64_t VirtualMachine::appStorageUsage(const std::string& id) const{
    ...
    for(auto it=std::filesystem::recursive_directory_iterator(appDataRoot(id),std::filesystem::directory_options::skip_permission_denied,ec);it!=std::filesystem::recursive_directory_iterator();it.increment(ec)){ if(ec){ec.clear();continue;} if(it->is_regular_file(ec)) bytes+=static_cast<uint64_t>(it->file_size(ec)); }
    return bytes;
}
...
std::string VirtualMachine::storageWrite(const std::string&id,const std::string&bucket,const std::string&path,const std::string&text){ if(installed_.find(id)==installed_.end()) return "APP NOT REGISTERED"; if(!hasPermission(id,"storage")) return "PERMISSION DENIED: STORAGE"; auto p=storagePath(id,bucket,path); if(p.empty()||!validStoragePath(p)||std::filesystem::is_symlink(p)) return "ACCESS DENIED"; ensureAppStorage(id); if(std::filesystem::is_directory(p)) return "NOT A FILE"; uint64_t before=std::filesystem::exists(p)?static_cast<uint64_t>(std::filesystem::file_size(p)):0,after=static_cast<uint64_t>(text.size()),used=appStorageUsage(id); if(used-before+after>installed_.at(id).quotaBytes) return "STORAGE QUOTA EXCEEDED"; std::filesystem::create_directories(p.parent_path()); std::ofstream f(p,std::ios::binary|std::ios::trunc); if(!f)return "WRITE FAILED"; f<<text; return f?"STORAGE WROTE":"WRITE FAILED"; }
...
void VirtualMachine::persistState() const{ std::ofstream f(root_/"system"/"runtime.state",std::ios::trunc);if(!f)return;f<<"LYCAN-RUNTIME 2\nram_bytes="<<ramBytes_<<"\nnetwork="<<(network_?"on":"off")<<"\nnext_pid="<<nextPid_<<"\ngeneration="<<generation_<<"\npackages=";bool first=true;for(const auto&[id,a]:installed_){if(!first)f<<';';first=false;f<<id<<','<<a.version<<','<<a.permissions<<','<<a.quotaBytes;}f<<"\n"; }
void VirtualMachine::loadState(){ installed_.clear();std::ifstream f(root_/"system"/"runtime.state");if(!f)return;std::string line,header;std::getline(f,header);if(header!="LYCAN-RUNTIME 2")return;while(std::getline(f,line)){auto eq=line.find('=');if(eq==std::string::npos)continue;auto key=line.substr(0,eq),value=line.substr(eq+1);try{if(key=="ram_bytes")ramBytes_=std::max<uint64_t>(128ULL*1024*1024,std::min<uint64_t>(8ULL*1024*1024*1024,std::stoull(value)));else if(key=="network")network_=value=="on";else if(key=="next_pid")nextPid_=static_cast<uint32_t>(std::stoul(value));else if(key=="generation")generation_=std::stoull(value);else if(key=="packages"){std::stringstream ss(value);std::string item;while(std::getline(ss,item,';')){std::stringstream row(item);std::string id,v,p,q;if(std::getline(row,id,',')&&std::getline(row,v,',')&&std::getline(row,p,',')&&std::getline(row,q,','))installed_[id]={id,v,p,std::stoull(q)};}}}catch(...){installed_.clear();return;}} }
...
std::string VirtualMachine::execute(const std::string&c){
    ...
    if(c=="version")return "LYCAN OS 1.1.1\nARES VIRTUAL CORE 1.1\nGUEST ABI 2";
