...
std::string clean(const std::string& s){ auto a=s.find_first_not_of(" \t"); auto b=s.find_last_not_of(" \t"); return a==std::string::npos?"":s.substr(a,b-a+1); }
...
void VirtualMachine::boot(){
    ...
    processes_.clear(); usedMemoryBytes_=0;
    processes_.push_back({1,"init","RUNNING","",8ULL*1024*1024});
    processes_.push_back({2,"desktop","RUNNING","",24ULL*1024*1024});
    usedMemoryBytes_=32ULL*1024*1024;
    if(installed_.empty()){
        const uint64_t q=16ULL*1024*1024;
        installed_={
            {"lycan-terminal",{"lycan-terminal","1.0.0","storage",q}},
            {"lycan-files",{"lycan-files","1.0.0","storage",q}},
            {"lycan-web",{"lycan-web","1.0.0","network,external",q}},
            {"lycan-store",{"lycan-store","1.0.0","storage,network",q}},
            {"lycan-settings",{"lycan-settings","1.0.0","storage",q}},
            {"lycan-diagnostics",{"lycan-diagnostics","1.0.0","storage",q}},
            {"lycan-snapshots",{"lycan-snapshots","1.0.0","storage",q}},
            {"crawford",{"crawford","1.0.0","storage",q}}
        };
...
bool VirtualMachine::validStoragePath(const std::filesystem::path& p) const{
    ...
    if(a||b) return false;
    const auto rel=target.lexically_relative(base); return !hasParentTraversal(rel);
}
std::filesystem::path VirtualMachine::storagePath(const std::string& id,const std::string& bucket,const std::string& path) const{
    ...
    std::string q=path.empty()?".":path; while(!q.empty()&&(q[0]=='/'||q[0]=='\\')) q.erase(q.begin());
    return appDataRoot(id)/bucket/q;
}
...
std::string VirtualMachine::storageList(const std::string&id,const std::string&bucket,const std::string&path) const{
    if(installed_.find(id)==installed_.end()) return "APP NOT REGISTERED"; if(!hasPermission(id,"storage")) return "PERMISSION DENIED: STORAGE"; ensureAppStorage(id);
    auto p=storagePath(id,bucket,path); if(p.empty()||!validStoragePath(p)||std::filesystem::is_symlink(p)) return "ACCESS DENIED"; if(!std::filesystem::exists(p)) return "PATH NOT FOUND"; if(!std::filesystem::is_directory(p)) return "NOT A DIRECTORY";
    std::string o; for(const auto&e:std::filesystem::directory_iterator(p)) o+=(e.is_directory()?"DIR  ":"FILE ")+std::string("  ")+e.path().filename().string()+"\n"; return o.empty()?"(empty)":o;
}
std::string VirtualMachine::storageRead(const std::string&id,const std::string&bucket,const std::string&path) const{
    if(installed_.find(id)==installed_.end()) return "APP NOT REGISTERED"; if(!hasPermission(id,"storage")) return "PERMISSION DENIED: STORAGE";
    auto p=storagePath(id,bucket,path); if(p.empty()||!validStoragePath(p)||std::filesystem::is_symlink(p)) return "ACCESS DENIED"; if(!std::filesystem::is_regular_file(p)) return "FILE NOT FOUND";
    std::ifstream f(p,std::ios::binary); std::ostringstream s; s<<f.rdbuf(); return s.str();
}
std::string VirtualMachine::storageWrite(const std::string&id,const std::string&bucket,const std::string&path,const std::string&text){
    if(installed_.find(id)==installed_.end()) return "APP NOT REGISTERED"; if(!hasPermission(id,"storage")) return "PERMISSION DENIED: STORAGE";
    auto p=storagePath(id,bucket,path); if(p.empty()||!validStoragePath(p)||std::filesystem::is_symlink(p)) return "ACCESS DENIED"; ensureAppStorage(id); if(std::filesystem::is_directory(p)) return "NOT A FILE";
    uint64_t before=std::filesystem::exists(p)?static_cast<uint64_t>(std::filesystem::file_size(p)):0, after=static_cast<uint64_t>(text.size()), used=appStorageUsage(id); if(used-before+after>installed_.at(id).quotaBytes) return "STORAGE QUOTA EXCEEDED";
    std::filesystem::create_directories(p.parent_path()); std::ofstream f(p,std::ios::binary|std::ios::trunc); if(!f) return "WRITE FAILED"; f<<text; return f?"STORAGE WROTE":"WRITE FAILED";
}
std::string VirtualMachine::storageDelete(const std::string&id,const std::string&bucket,const std::string&path){
    if(installed_.find(id)==installed_.end()) return "APP NOT REGISTERED"; if(!hasPermission(id,"storage")) return "PERMISSION DENIED: STORAGE"; auto p=storagePath(id,bucket,path);
    if(p.empty()||!validStoragePath(p)||std::filesystem::is_symlink(p)) return "ACCESS DENIED"; ensureAppStorage(id); if(p==appDataRoot(id)/bucket) return "REFUSED: STORAGE ROOT"; if(!std::filesystem::exists(p)) return "PATH NOT FOUND";
    std::error_code ec; std::filesystem::remove_all(p,ec); return ec?"DELETE FAILED":"STORAGE DELETED";
}
...
void VirtualMachine::persistState() const{
    std::ofstream f(root_/"system"/"runtime.state",std::ios::trunc); if(!f) return;
    f<<"LYCAN-RUNTIME 2\nram_bytes="<<ramBytes_<<"\nnetwork="<<(network_?"on":"off")<<"\nnext_pid="<<nextPid_<<"\ngeneration="<<generation_<<"\npackages=";
    bool first=true; for(const auto&[id,a]:installed_){ if(!first) f<<';'; first=false; f<<id<<','<<a.version<<','<<a.permissions<<','<<a.quotaBytes; } f<<"\n";
}
void VirtualMachine::loadState(){
    installed_.clear(); std::ifstream f(root_/"system"/"runtime.state"); if(!f) return; std::string line,header; std::getline(f,header); if(header!="LYCAN-RUNTIME 2") return;
    while(std::getline(f,line)){ auto eq=line.find('='); if(eq==std::string::npos) continue; auto key=line.substr(0,eq),value=line.substr(eq+1); try{
        if(key=="ram_bytes") ramBytes_=std::max<uint64_t>(128ULL*1024*1024,std::min<uint64_t>(8ULL*1024*1024*1024,std::stoull(value)));
        else if(key=="network") network_=value=="on";
        else if(key=="next_pid") nextPid_=static_cast<uint32_t>(std::stoul(value));
        else if(key=="generation") generation_=std::stoull(value);
        else if(key=="packages"){ std::stringstream ss(value); std::string item; while(std::getline(ss,item,';')){ std::stringstream row(item); std::string id,v,p,q; if(std::getline(row,id,',')&&std::getline(row,v,',')&&std::getline(row,p,',')&&std::getline(row,q,',')) installed_[id]={id,v,p,std::stoull(q)}; } }
    }catch(...){ installed_.clear(); return; }}
}
...
std::string VirtualMachine::ps() const{ std::string o="PID   NAME              STATE       MEMORY\n-----------------------------------------------\n"; for(const auto&p:processes_) o+=std::to_string(p.pid)+"   "+p.name+"              "+p.state+"       "+std::to_string(p.memoryBytes/1024)+" KB"+(p.error.empty()?"":"  ERROR="+p.error)+"\n"; return o; }
...
std::string VirtualMachine::setRam(const std::string&mb){ try{ auto n=std::stoull(mb)*1024ULL*1024ULL; if(n<128ULL*1024*1024||n>8ULL*1024*1024*1024) return "RAM MUST BE 128-8192 MB"; if(n<usedMemoryBytes_) return "RAM BELOW CURRENT USAGE"; ramBytes_=n; ++generation_; persistState(); return "RAM SET "+std::to_string(n/1048576ULL)+" MB"; }catch(...){return "INVALID RAM";} }

std::string VirtualMachine::registerApp(const std::string&spec){
    std::stringstream ss(spec); std::string id,v,q,perms; if(!(ss>>id>>v>>q)) return "USAGE: app register <id> <version> <quota-mb> [permissions]"; std::getline(ss,perms); perms=clean(perms); if(!perms.empty()&&perms.front()=='['&&perms.back()==']') perms=perms.substr(1,perms.size()-2); if(!validAppId(id)) return "INVALID APP ID";
    try{ auto quota=std::stoull(q)*1024ULL*1024ULL; if(quota<1024ULL*1024||quota>1024ULL*1024*1024) return "APP QUOTA MUST BE 1-1024 MB"; for(char c:perms) if(!(std::isalnum(static_cast<unsigned char>(c))||c==','||c=='-'||c=='_')) return "INVALID PERMISSIONS"; if(perms.find("storage")==std::string::npos && perms.size()==0) perms="storage"; installed_[id]={id,v,perms,quota}; ensureAppStorage(id); persistState(); return "APP REGISTERED "+id+" "+v; }catch(...){return "INVALID APP REGISTRATION";}
}
std::string VirtualMachine::unregisterApp(const std::string&id){ if(installed_.find(id)==installed_.end()) return "APP NOT REGISTERED"; if(id.rfind("lycan-",0)==0||id=="crawford") return "REFUSED: CORE APP"; for(const auto&p:processes_) if(p.name==id&&p.pid>2) return "REFUSED: APP RUNNING"; installed_.erase(id); std::error_code ec; std::filesystem::remove_all(appDataRoot(id),ec); persistState(); return "APP UNREGISTERED "+id; }
std::string VirtualMachine::appState(const std::string&id) const{ auto ai=installed_.find(id); if(ai==installed_.end()) return "APP NOT REGISTERED"; auto it=std::find_if(processes_.begin(),processes_.end(),[&](const Process&p){return p.name==id&&p.pid>2;}); if(it==processes_.end()) return "APP STATE           STOPPED\nAPP                 "+id+"\nVERSION             "+ai->second.version; std::ostringstream o; o<<"APP STATE           "<<it->state<<"\nAPP                 "<<id<<"\nVERSION             "<<ai->second.version<<"\nPID                 "<<it->pid; if(!it->error.empty()) o<<"\nERROR               "<<it->error; return o.str(); }

std::string VirtualMachine::diagnostics() const{
    size_t files=0,dirs=0; uint64_t bytes=0; std::error_code ec;
    if(std::filesystem::exists(root_,ec)){ for(auto it=std::filesystem::recursive_directory_iterator(root_,std::filesystem::directory_options::skip_permission_denied,ec);it!=std::filesystem::recursive_directory_iterator();it.increment(ec)){ if(ec){ec.clear();continue;} if(it->is_regular_file(ec)){++files;bytes+=static_cast<uint64_t>(it->file_size(ec));} else if(it->is_directory(ec))++dirs; } }
    size_t snaps=0; if(std::filesystem::exists(root_/"snapshots",ec)) for(const auto&i:std::filesystem::directory_iterator(root_/"snapshots",ec)) if(i.path().extension()==".snap") ++snaps;
    std::ostringstream o; o<<asciiLogo()<<"LYCAN DIAGNOSTIC CORE\n=====================\nRUNTIME             ONLINE\nARES CPU            ONLINE\nVIRTUAL RAM         "<<ramBytes_/1048576ULL<<" MB\nMEMORY USED        "<<usedMemoryBytes_/1048576ULL<<" MB\nLYFS                ISOLATED\nGUEST ROOT          LOCALIZED\nHOST ACCESS         DENIED\nSYMLINK POLICY      RESTRICTED\nNETWORK             "<<(network_?"ONLINE":"OFFLINE")<<"\nVNET                "<<(network_?"UP":"DOWN")<<"\nPROCESSES           "<<processes_.size()<<"\nPACKAGES            "<<installed_.size()<<"\nGUEST FILES         "<<files<<"\nGUEST DIRECTORIES   "<<dirs<<"\nGUEST DATA          "<<bytes/1024<<" KB\nSNAPSHOTS           "<<snaps<<"\nVM CYCLES           "<<cycles_<<"\nRUNTIME GENERATION  "<<generation_<<"\nSECURITY            ENFORCED\n"; return o.str();
}
...
std::string VirtualMachine::snapshotRestore(const std::string&name){
    if(!validSnapshotName(name))return "INVALID SNAPSHOT NAME"; std::ifstream f(snapshotPath(name)); if(!f)return "SNAPSHOT NOT FOUND"; std::string line,header; std::getline(f,header); if(header!="LYCAN-SNAPSHOT 2")return "INVALID SNAPSHOT FORMAT";
    uint64_t cycles=cycles_,ram=ramBytes_,gen=generation_; uint32_t pid=nextPid_; bool net=network_; std::map<std::string,AppRecord> packages;
    ...
    auto fs=snapshotFsPath(name); if(!std::filesystem::exists(fs)) return "SNAPSHOT FILESYSTEM MISSING";
    std::error_code ec; for(const char* top:{"home","apps","appdata","system"}){std::filesystem::remove_all(root_/top,ec); if(std::filesystem::exists(fs/top,ec)) std::filesystem::copy(fs/top,root_/top,std::filesystem::copy_options::recursive|std::filesystem::copy_options::overwrite_existing,ec); if(ec)return "SNAPSHOT FILESYSTEM RESTORE FAILED";}
    cycles_=cycles;ramBytes_=ram;nextPid_=pid;generation_=gen;network_=net;installed_=std::move(packages);processes_.clear();usedMemoryBytes_=0;processes_.push_back({1,"init","RUNNING","",8ULL*1024*1024});processes_.push_back({2,"desktop","RUNNING","",24ULL*1024*1024});usedMemoryBytes_=32ULL*1024*1024;persistState();return "SNAPSHOT RESTORED "+name;
}
...
bool VirtualMachine::validGuestPath(const std::filesystem::path&p) const{std::error_code a,b;const auto base=std::filesystem::weakly_canonical(root_,a),target=std::filesystem::weakly_canonical(p,b);if(a||b)return false;const auto rel=target.lexically_relative(base);if(hasParentTraversal(rel))return false;auto m=std::mismatch(base.begin(),base.end(),target.begin(),target.end());return m.first==base.end();}
bool VirtualMachine::isGuestRoot(const std::filesystem::path&p) const{std::error_code a,b;const auto base=std::filesystem::weakly_canonical(root_,a),target=std::filesystem::weakly_canonical(p,b);return !a&&!b&&base==target;}
...
std::string VirtualMachine::execute(const std::string&c){
    ...
    if(c=="version") return "LYCAN OS 1.1.0\nARES VIRTUAL CORE 1.1\nGUEST ABI 2";
    if(c=="help") return "logo | ping | version | help | diagnostics | memory | vm ram <mb> | pwd | ls [path] | tree [path] | cat <path> | write <path> <text> | mkdir <path> | touch <path> | rm <path> | apps | app register <id> <version> <quota-mb> [permissions] | app state <id> | app unregister <id> | ps | suspend <app> | resume <app> | crash <app> [error] | network [on|off|status|interfaces] | network app <id> | storage <id> <data|config|cache> <path> | storage-read <id> <bucket> <path> | storage-write <id> <bucket> <path> <text> | storage-delete <id> <bucket> <path> | storage-usage <id> | storage-quota <id> | open <app> | close <app> | snapshots | snapshot <name> | snapshot-info <name> | restore <name> | delete-snapshot <name> | web start | web tab <url>";
    if(c=="logo")return asciiLogo(); if(c=="pwd")return "/home"; if(c=="diagnostics")return diagnostics(); if(c=="memory")return memoryStatus(); if(c=="ls")return ls("/home"); if(c.rfind("ls ",0)==0)return ls(c.substr(3)); if(c=="tree")return tree("/home"); if(c.rfind("tree ",0)==0)return tree(c.substr(5)); if(c=="apps")return apps(); if(c=="ps")return ps();
    ...
    if(c.rfind("vm ram ",0)==0)return setRam(clean(c.substr(7)));
    if(c.rfind("app register ",0)==0)return registerApp(c.substr(13)); if(c.rfind("app unregister ",0)==0)return unregisterApp(clean(c.substr(15))); if(c.rfind("app state ",0)==0)return appState(clean(c.substr(10)));
    ...
    if(c.rfind("mkdir ",0)==0){auto p=guestPath(c.substr(6));if(!validGuestPath(p)||isGuestRoot(p))return "ACCESS DENIED";std::error_code e;std::filesystem::create_directories(p,e);return e?"CREATE FAILED":"DIRECTORY CREATED";}
    if(c.rfind("touch ",0)==0){auto p=guestPath(c.substr(6));if(!validGuestPath(p)||isGuestRoot(p))return "ACCESS DENIED";std::ofstream f(p);return f?"FILE CREATED":"CREATE FAILED";}
    if(c.rfind("write ",0)==0){auto s=c.substr(6);auto sp=s.find(' ');if(sp==std::string::npos)return "USAGE: write <path> <text>";auto p=guestPath(s.substr(0,sp));if(!validGuestPath(p)||isGuestRoot(p)||std::filesystem::is_symlink(p))return "ACCESS DENIED";std::filesystem::create_directories(p.parent_path());std::ofstream f(p,std::ios::trunc);if(!f)return "WRITE FAILED";f<<s.substr(sp+1);return "WROTE "+p.filename().string();}
    if(c.rfind("cat ",0)==0){auto p=guestPath(c.substr(4));if(!validGuestPath(p)||std::filesystem::is_symlink(p))return "ACCESS DENIED: OUTSIDE LYFS";if(!std::filesystem::is_regular_file(p))return "FILE NOT FOUND";std::ifstream f(p);std::ostringstream s;s<<f.rdbuf();return s.str();}
    if(c.rfind("rm ",0)==0){auto p=guestPath(c.substr(3));if(isGuestRoot(p))return "REFUSED: LYFS ROOT IS IMMUTABLE";if(!validGuestPath(p)||std::filesystem::is_symlink(p))return "ACCESS DENIED";if(!std::filesystem::exists(p))return "PATH NOT FOUND";std::error_code e;std::filesystem::remove_all(p,e);return e?"DELETE FAILED":"DELETED";}
    if(c.rfind("open ",0)==0){auto id=clean(c.substr(5));auto ai=installed_.find(id);if(ai==installed_.end())return "APP NOT FOUND";auto it=std::find_if(processes_.begin(),processes_.end(),[&](const Process&p){return p.name==id&&p.pid>2;});if(it!=processes_.end()){if(it->state=="SUSPENDED"){it->state="RUNNING";it->error.clear();persistState();return "APP RESUMED "+id;} if(it->state=="CRASHED")return "APP CRASHED";return "APP ALREADY RUNNING";} const uint64_t mem=16ULL*1024*1024;if(usedMemoryBytes_+mem>ramBytes_)return "OUT OF VIRTUAL MEMORY";processes_.push_back({nextPid_++,id,"RUNNING","",mem});usedMemoryBytes_+=mem;persistState();return "OPENED "+id;}
    if(c.rfind("suspend ",0)==0){auto id=clean(c.substr(8));auto it=std::find_if(processes_.begin(),processes_.end(),[&](const Process&p){return p.name==id&&p.pid>2;});if(it==processes_.end())return "APP NOT RUNNING";if(it->state=="CRASHED")return "APP CRASHED";it->state="SUSPENDED";persistState();return "SUSPENDED "+id;}
    if(c.rfind("resume ",0)==0){auto id=clean(c.substr(7));auto it=std::find_if(processes_.begin(),processes_.end(),[&](const Process&p){return p.name==id&&p.pid>2;});if(it==processes_.end())return "APP NOT RUNNING";if(it->state=="CRASHED")return "APP CRASHED";it->state="RUNNING";persistState();return "RESUMED "+id;}
    if(c.rfind("crash ",0)==0){auto s=c.substr(6);auto sp=s.find(' ');auto id=sp==std::string::npos?s:s.substr(0,sp);auto err=sp==std::string::npos?"Unspecified application failure":clean(s.substr(sp+1));auto it=std::find_if(processes_.begin(),processes_.end(),[&](const Process&p){return p.name==id&&p.pid>2;});if(it==processes_.end())return "APP NOT RUNNING";it->state="CRASHED";it->error=err;persistState();return "APP CRASHED "+id;}
    if(c.rfind("close ",0)==0){auto id=clean(c.substr(6));auto it=std::find_if(processes_.begin(),processes_.end(),[&](const Process&p){return p.name==id&&p.pid>2;});if(it==processes_.end())return "APP NOT RUNNING";usedMemoryBytes_=usedMemoryBytes_>=it->memoryBytes?usedMemoryBytes_-it->memoryBytes:0;processes_.erase(it);persistState();return "CLOSED "+id;}
