...
void AppHost::boot(){fs_.format();security_.trustPublisher("LYCAN");processes_.spawn("init",8192);processes_.spawn("desktop",16384);vm_.boot();std::string welcome;if(!fs_.readText("/home/Welcome.txt",welcome))fs_.writeText("/home/Welcome.txt","Welcome to LYCAN OS 1.0\nThis is a virtual guest environment hosted by Windows.\n");}
std::string AppHost::execute(const std::string&cmd){
 ...
 if(cmd=="apps"){
  auto list=packages_.installed();if(list.empty())return "(no packages installed)";
  std::string s="ID                 VERSION    PUBLISHER    LOCATION                 PERMISSIONS\n";
  s+="--------------------------------------------------------------------------------\n";
 ...
 if(cmd.rfind("install ",0)==0){const auto id=cmd.substr(8);const Package catalog[]={{"lycan-terminal","Terminal","1.0.0","LYCAN","Native ARES guest command workspace",""},{"lycan-files","Files","1.0.0","LYCAN","LYFS guest file manager",""},{"lycan-web","Web","1.0.0","LYCAN","Mozilla Gecko web surface",""},{"lycan-snapshots","Snapshots","1.0.0","LYCAN","Save and restore guest state",""},{"lycan-diagnostics","Diagnostics","1.0.0","LYCAN","VM health and security inspection",""},{"crawford","Crawford","1.0.0","LYCAN","AI integration boundary for LYCAN apps",""}};for(const auto&p:catalog)if(p.id==id)return packages_.install(p)?"installed "+id:"installation rejected";return "package not found";}
