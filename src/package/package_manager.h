...
public:
 ...
 bool inspectArchive(const std::filesystem::path&,Package&,std::string* error=nullptr) const; bool uninstall(const std::string& id); bool rollback(const std::string& id); std::vector<Package> installed() const; static bool validId(const std::string&);
private:
