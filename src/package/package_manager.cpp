...
bool PackageManager::install(const Package&p){
    ...
    return(bool)db&&(db<<p.id<<"|"<<p.version<<"|"<<p.publisher<<"\n");
}
...
std::string PackageManager::packageSha256(const std::filesystem::path&package) {
    return PackageArchive::sha256File(package);
}

bool PackageManager::verifySha256(const std::string&calculated,const std::string&expected) {
    // An absent/malformed catalog digest is a hard failure. Never treat it as verified.
...
bool PackageManager::installArchive(const std::filesystem::path&path,const std::string&expectedSha256,std::string*error){
    ...
    // and only then inspect/extract it. No guest filesystem mutation occurs before this check.
    ...
    if(!PackageArchive::inspect(path,manifest,entries,e)||!PackageArchive::extract(path,target,manifest,e)){
        if(error)*error=e;return false;
    }
    std::ofstream db(db_,std::ios::app);
    if(!db||(db<<p.id<<"|"<<p.version<<"|"<<p.publisher<<"\n").fail()){
        if(error)*error="cannot update package database";return false;
    }
    return true;
}
...
std::vector<Package> PackageManager::installed()const{
    ...
    for(auto&e:fs_.list("/apps"))if(e.directory){
        Package p;p.id=e.path.substr(6);p.name=p.id;p.version="1.0.0";p.publisher="LYCAN";
        std::string j;
        if(fs_.readText("/apps/"+p.id+"/manifest.json",j)){
            auto value=[&](const std::string&k){
                auto n="\""+k+"\"";auto a=j.find(n);if(a==std::string::npos)return std::string{};
                a=j.find(':',a+n.size());auto q=j.find('"',a);auto z=j.find('"',q+1);
                return q==std::string::npos||z==std::string::npos?std::string{}:j.substr(q+1,z-q-1);
            };
