...
bool PackageManager::install(const Package&p){
    ...
    auto target=fs_.hostPath("/apps/"+p.id);
    auto cache=fs_.hostPath("/package-cache");
    std::error_code ec;
    ...
    const auto cleanup=[&]{std::error_code x;std::filesystem::remove_all(stage,x);std::filesystem::remove_all(backup,x);};
    std::string m="{\n  \"id\": \""+p.id+"\",\n  \"name\": \""+p.name+"\",\n  \"version\": \""+p.version+"\",\n  \"publisher\": \""+p.publisher+"\",\n  \"entry\": \""+p.entry+"\"\n}\n";
    if(!fs_.writeText("/package-cache/"+stage.filename().string()+"/manifest.json",m)){cleanup();return false;}

    std::error_code renameEc;
    bool hadPrevious=std::filesystem::exists(target);
    if(hadPrevious){std::filesystem::rename(target,backup,renameEc);if(renameEc){cleanup();return false;}}
    std::filesystem::rename(stage,target,renameEc);
    if(renameEc){if(hadPrevious){std::error_code restoreEc;std::filesystem::rename(backup,target,restoreEc);}cleanup();return false;}

    std::ofstream db(db_,std::ios::app);
    if(!db||(db<<p.id<<"|"<<p.version<<"|"<<p.publisher<<"\n").fail()){
        std::error_code x;std::filesystem::remove_all(target,x);
        if(hadPrevious)std::filesystem::rename(backup,target,x);
...
bool PackageManager::installArchiveFromCatalog(const std::filesystem::path&package,const std::string&catalogJson,std::string*error){
    ...
    if(!isSha256(expected)){
        if(error)*error=securityFailure();
        return false;
    }
...
bool PackageManager::installArchive(const std::filesystem::path&path,const std::string&expectedSha256,std::string*error){
    // TRANSACTION ORDER:
    // 1) verify archive, 2) extract into isolated staging, 3) validate the staged
    // manifest/publisher, 4) move the old app to backup, 5) atomically promote the
    // staged app, 6) update package state. Any failure restores the old version.
    ...
    const auto rollbackFs=[&]{
        std::error_code x;
        if(committed)std::filesystem::remove_all(target,x);
        else if(std::filesystem::exists(target))std::filesystem::remove_all(target,x);
        if(hadPrevious && !std::filesystem::exists(target))std::filesystem::rename(backup,target,x);
        std::filesystem::remove_all(stage,x);
        if(std::filesystem::exists(backup) && !std::filesystem::exists(target))std::filesystem::rename(backup,target,x);
    };

    PackageManifest stagedManifest;std::vector<PackageEntry> stagedEntries;
    if(!PackageArchive::extract(path,stage,stagedManifest,e)){
        rollbackFs();if(error)*error=transactionFailure(e);return false;
    }

    // Validate what was actually staged, not merely what the archive advertised.
    if(stagedManifest.id!=p.id || stagedManifest.version!=p.version ||
       stagedManifest.publisher!=p.publisher ||
       !validId(stagedManifest.id) ||
       stagedManifest.entry.rfind("/apps/"+p.id+"/",0)!=0 ||
       !security_.publisherTrusted(stagedManifest.publisher)){
        rollbackFs();if(error)*error=transactionFailure("staged manifest validation failed");return false;
    }
    const auto stagedEntry=fs_.hostPath(stagedManifest.entry);
    if(!std::filesystem::exists(stagedEntry)){
        rollbackFs();if(error)*error=transactionFailure("staged entry point is missing");return false;
    }

    hadPrevious=std::filesystem::exists(target);
    if(hadPrevious){
        std::filesystem::rename(target,backup,ec);
        if(ec){rollbackFs();if(error)*error=transactionFailure(ec.message());return false;}
    }
    std::filesystem::rename(stage,target,ec);
    if(ec){rollbackFs();if(error)*error=transactionFailure(ec.message());return false;}
    committed=true;

    // Persist the new record only after the filesystem commit. If persistence fails,
    // remove the new version and restore the backup, so the guest never reports a
    // package as installed when its package database was not updated.
    std::ofstream db(db_,std::ios::app);
    if(!db||(db<<p.id<<"|"<<p.version<<"|"<<p.publisher<<"\n").fail()){
        rollbackFs();if(error)*error=transactionFailure("cannot update package database");return false;
    }

    std::filesystem::remove_all(backup,ec);
    if(ec){
        // The new version is already committed and recorded. Backup cleanup failing
        // must not destroy the working installation; leave the backup for recovery.
        if(error)*error="LYCAN PACKAGE MANAGER\n\nInstallation completed with a recoverable backup retained.\n\nReason:\n"+ec.message();
        return true;
    }
    return true;
}
